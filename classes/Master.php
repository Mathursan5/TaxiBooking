	...
	function save_cab(){
		...
		if(empty($_POST['id'])){
			...
			while(true){
				...
				if($check > 0){
					$code = "100" +1;//sprintf("%'.05d",ceil($code) + 1);
				}else{
					break;
				}
			}
		...
		//DriverName
		if(isset($cab_driver)){
			$check = $this->conn->query("SELECT * FROM `cab_list` where `cab_driver` = '{$cab_driver}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
			if($this->capture_err())
				return $this->capture_err();
			if($check > 0){
				$resp['status'] = 'failed';
				$resp['msg'] = " Driver already exist.";
				return json_encode($resp);
				exit;
			}
		}

...
<section class="py-5">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-10">
                <div class="card bg-dark text-white shadow rounded-0">
                    <div class="card-body">
                        <h1 class="display-6 text-center fw-bolder mb-4">Available Taxi</h1>
                        <hr class="bg-success">
                        <div class="form-group">
                            <div class="input-group mb-3">
                                <input type="search" id="search" class="form-control bg-dark text-white" placeholder="Search Here..." aria-label="Search Here" aria-describedby="basic-addon2">
                                <div class="input-group-append">
                                    <span class="input-group-text bg-warning" id="basic-addon2"><i class="fa fa-search text-white"></i></span>
                                </div>
                            </div>
                            <hr class="bg-success">
                        </div>
                        <div class="row row-cols-1 row-cols-md-2 row-cols-xl-3" id="cab_list">
                            <?php 
                            $cabs = $conn->query("SELECT c.*, cc.name as category FROM `cab_list` c inner join category_list cc on c.category_id = cc.id where c.delete_flag = 0 order by c.`reg_code`");
                            ...
                            <a class="col item text-decoration-none text-dark book_cab" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>" data-bodyno="<?php echo $row['body_no'] ?>"<?php echo $isBusy ? 'style="pointer-events: none; opacity: 0.9;"' : ''; ?>>
                                <div class="callout <?php echo $isBusy ? 'callout-secondary' : 'callout-primary'; ?> border-warning rounded-0">
                                    <dl>
                                        <dt class="h5"> Driver: <?php echo $row['cab_driver'] ?></dt>
                                        <dt>
                                            <?php 
                                                switch($row['category']){
                                                    case "Tuk":
